...
<header class="bg-slate-900 text-white px-2 sm:px-4 py-2 flex items-center justify-between z-40 border-b border-white/10 shadow-md shrink-0">
  <div class="flex items-center gap-2 sm:gap-4 overflow-hidden">
    ...
    {/if}
    <button onclick={(e) => { e.preventDefault(); uiStore.currentView = "library"; }} class="p-2 sm:p-2.5 hover:bg-white/10 rounded-lg transition-colors" title="Back to Library ( L )">
  ...
  <div class="flex items-center gap-2 sm:gap-3">
    {#if uiStore.currentView === "reader"}
      ...
      <div class="flex bg-white/5 rounded-lg p-1 border border-white/10">
        <button onclick={(e) => { e.preventDefault(); uiStore.layoutMode = "side-by-side"; }} title="Side by Side" class="p-1.5 sm:p-2 rounded-md {uiStore.layoutMode === "side-by-side" ? "bg-white/20" : "hover:bg-white/10"}"><svg class="w-4 h-4 sm:w-5 sm:h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M3 5h8v14H3V5zm10 0h8v14h-8V5z"/></svg></button>
        <button onclick={(e) => { e.preventDefault(); uiStore.layoutMode = "over-under"; }} title="Stacked" class="p-1.5 sm:p-2 rounded-md {uiStore.layoutMode === "over-under" ? "bg-white/20" : "hover:bg-white/10"}"><svg class="w-4 h-4 sm:w-5 sm:h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M3 5h18v6H3V5zm0 8h18v6H3v-6z"/></svg></button>
        <button onclick={(e) => { e.preventDefault(); uiStore.layoutMode = "plain"; }} title="Single" class="p-1.5 sm:p-2 rounded-md {uiStore.layoutMode === "plain" ? "bg-white/20" : "hover:bg-white/10"}"><svg class="w-4 h-4 sm:w-5 sm:h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M3 5h18v2H3V5zm0 4h18v2H3V9zm0 4h18v2H3v-2zm0 4h18v2H3v-2z"/></svg></button>
      </div>
